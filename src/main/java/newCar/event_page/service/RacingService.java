...
public class RacingService {
    ...
    public void drawWinners(List<WinnerSettingDTO> winnerSettingDTOList, Long eventId) {
        ...
        for (WinnerSettingDTO winnerSettingDTO : winnerSettingDTOList) {
            ...
            for (int i = 0; i < numberOfWinners; i++) {
                Participant winner = getWinner(participantSet,totalWeight);
                totalWeight = totalWeight  - winner.weight; //전체가중치에서 당첨자의 가중치를 빼준다
    ...
    private Participant getWinner(Set<Participant> participantSet,double totalWeight){
